...
const AdminCourseEdit = ({ course, isOpen, onClose, onSuccess }) => {
  const { user } = useAuth();
  ...
  const handleInputChange = (field, value) => {
    ...
    const hasFieldChanged =
      field === 'is_active'
        ? currentValue !== originalData[field]
      ...
      setHasChanges(() => {
        const newHasChanges = Object.keys(formData).some(key =>
          ...
          (key === 'is_active'
            ...
            : String(formData[key] || '') !== String(originalData[key] || ''))
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Actualizar el curso
      const updatedCourse = await courseService.updateCourse(course.id, sanitizedData);

      // Registrar auditoría
      await courseService.logAdminAction('update_course', updatedCourse.id, user.id, {
      ...
      const errorMessage = courseService.handleAdminError(error, 'update');
      setErrors({ submit: errorMessage });
      showError(`Error al actualizar el curso: ${errorMessage}`);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
      <div className="relative top-4 mx-auto p-5 border w-full max-w-4xl shadow-lg rounded-md bg-white">
        ...
        <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-200">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-gray-600">Modifique la información del curso: {course.title}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center space-x-4">
              <span className={`text-sm ${hasChanges ? 'text-blue-600' : 'text-gray-500'}`}>
          ...
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div
              className={`h-2 rounded-full transition-all duration-300 ${
                Object.keys(errors).length === 0 && hasChanges ? 'bg-blue-500' :
                Object.keys(errors).length === 0 ? 'bg-green-500' : 'bg-yellow-500'
              }`}
              style={{
                width: `${Math.max(0, 100 - (Object.keys(errors).length * 10))}%`
              }}
            ></div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Fechas y Recursos */}
          <div className="bg-gray-50 rounded-lg p-6">
            <h3 className="text-lg font-medium text-gray-900 mb-4">Fechas y Recursos</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Fecha de inicio */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
          ...
          {errors.submit && (
            <div className="bg-red-50 border border-red-200 rounded-md p-4">
              <div className="flex">
                ...
                <div className="ml-3">
                  <h3 className="text-sm font-medium text-red-800">Error al actualizar el curso</h3>
          ...
          <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
            <div className="relative top-4 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-md bg-white">
              <div className="flex items-center justify-between mb-4 pb-4 border-b border-gray-200">
                <h3 className="text-lg font-medium text-gray-900">Vista Previa de Cambios</h3>
                <button
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    <span className="font-medium">Instructor:</span> {instructors.find(i => i.id === parseInt(formData.instructor_id))?.name || 'No asignado'}
                  </div>
                  {formData.duration && (
                    <div>
                      <span className="font-medium">Duración:</span> {formData.duration} horas
                    </div>
                  )}
                  {formData.price && (
                    <div>
                      <span className="font-medium">Precio:</span> S/ {formData.price}
                    </div>
                  )}
                </div>
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-md p-3">
                  <div className="flex">
                    ...
                    <div className="ml-3">
                      <h4 className="text-sm font-medium text-blue-800">Cambios detectados</h4>
                      <p className="text-sm text-blue-700 mt-1">
                        Se actualizarán los campos modificados del curso.
                      </p>
                    </div>
                  </div>
                </div>
